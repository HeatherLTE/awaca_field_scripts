...
# to my laptop
# via the local network of the opu
# using rsync
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Transfer things from the AWACA Raid to my laptop over the local opu network")
    ...
    epfl_archive = os.path.join('/data/awaca_onsite_data/raid', site, instrument, what)
    print(epfl_archive)
    ...
    for day in days_to_sync:
        ...
        try:
            sync_laptop_raid(raid_folder, epfl_folder, site)
        except:
...
def sync_laptop_raid(raid_folder, epfl_folder, site):
    ...
    process = subprocess.Popen(
        [
            ...
            '--stats',
            '--verbose',
            f'{user_mira}@192.168.1.110:{raid_folder}', 
            epfl_folder
        ],
